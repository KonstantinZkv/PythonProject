...
def test_card_number_generator():
    ...
    for number in generated_numbers:
        ...
        assert number.count(" ") == 3  # Проверяем наличие пробелов
        assert all(part.isdigit() for part in number.split())  # Проверяем, что все части - цифры


def test_unique_card_numbers():
    gen = card_number_generator()

    generated_numbers = set(next(gen) for _ in range(1000))  # Генерируем 1000 номеров
    assert len(generated_numbers) == 1000  # Проверяем уникальность


def test_card_number_within_range():
    gen = card_number_generator(start=4000000000000000, stop=4999999999999999)

    for _ in range(10):
        number = next(gen)
        first_digit = int(number[0])  # Первый символ номера карты
        assert first_digit == 4  # Проверяем, что номер начинается с '4'
